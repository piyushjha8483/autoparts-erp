...
export default function QuotationsPage() {
  ...
  const convertToOrder = async (id, qNum) => {
    const orderNum = prompt("Enter Sales Order Number:");
    if (!orderNum) return;
    try {
      await api.post(`/quotations/${id}/convert`, { orderNumber: orderNum });
      alert("Converted successfully!");
      fetchData();
    } catch (err) {
      alert(err.response?.data?.message || 'Error converting');
    }
  };
              ...
              <tr key={quot.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                  ...
                  {isSales && quot.status === 'ACCEPTED' && !quot.salesOrder && (
                    <button onClick={() => convertToOrder(quot.id, quot.quotationNumber)} className="text-white bg-indigo-600 px-2 py-1 rounded hover:bg-indigo-700">Convert to SO</button>
                  )}
                </td>
              </tr>
            ))}
          ...
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateQuotation} className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <input required placeholder="Quotation Number" className="border p-2 rounded" value={formData.quotationNumber} onChange={e => setFormData({...formData, quotationNumber: e.target.value})} />
                <select required className="border p-2 rounded" value={formData.enquiryId} onChange={e => handleEnquirySelect(e.target.value)}>
                  <option value="">Select Enquiry</option>
                  {enquiries.map(e => <option key={e.id} value={e.id}>{e.enquiryNumber} - {e.customer?.companyName}</option>)}
                </select>
                <input required type="date" className="border p-2 rounded" value={formData.validUntil} onChange={e => setFormData({...formData, validUntil: e.target.value})} />
              </div>
              ...
              <div className="space-y-2">
                {formData.items.map((item, idx) => (
                  <div key={idx} className="flex space-x-2 items-center bg-gray-50 p-2 rounded">
                    ...
                    <input required type="number" placeholder="Qty" className="border p-1 rounded w-20" value={item.quantity} onChange={e => {
                      const newItems = [...formData.items]; newItems[idx].quantity = e.target.value; setFormData({...formData, items: newItems});
                    }} />
                    <input required type="number" placeholder="Price" className="border p-1 rounded w-24" value={item.unitPrice} onChange={e => {
                      const newItems = [...formData.items]; newItems[idx].unitPrice = e.target.value; setFormData({...formData, items: newItems});
                    }} />
                    <input required type="number" placeholder="Disc %" className="border p-1 rounded w-20" value={item.discountPercent} onChange={e => {
                      const newItems = [...formData.items]; newItems[idx].discountPercent = e.target.value; setFormData({...formData, items: newItems});
                    }} />
                    <input required type="number" placeholder="GST %" className="border p-1 rounded w-20" value={item.gstPercent} onChange={e => {
